...
export default function JobDetails() {
    ...
    useEffect(() => {
        ...
        checkApplied();
    }, [jobs, props.id]); // Added dependencies to ensure it re-runs when jobs or id changes

    const fetchJob = () => {
        const job = jobs.find(job => job.id == props.id);
        if (job) {
            setJob(job);
        }
    };
    ...
    const checkApplied = () => {
        ...
        const job = appliedJobs.find(job => job.jobId == props.id);
        if (job) setIsApplied(true);
    };
                    ...
                    <div className='col-sm-12 col-md-4'>
                        ...
                        <div className='shadow-sm p-2 pt-4 bg-white rounded pt-3 px-3 pb-4 right-side'>
                          ...
                          <div className='text-center'>
                                {jobSeeker && JSON.parse(localStorage.getItem('jwt-token'))?.holder === "JOBSEEKER" && (
                                    <button className='btn btn-danger' disabled={isApplied} onClick={handleApply}>
